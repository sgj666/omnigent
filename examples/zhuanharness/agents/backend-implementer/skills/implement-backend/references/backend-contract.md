...
Task Packet 每个仓库必须给出 `run_base_sha`、`launch_head_sha`、`materialize_sha`、`expected_base_sha`、`owned_paths` 和预分配 `handoff_ref`。共享 API/schema/消息/数据契约必须为 `FROZEN`，否则跨端实现不得并行。
