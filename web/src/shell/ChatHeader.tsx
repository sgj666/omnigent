      ...
      <div className={cn("flex items-center gap-1", !sidebarOpen && "traffic-light-clearance")}>
        {!sidebarOpen && (
          <Tooltip>
            ...
            <TooltipContent side="bottom">{t("shell.openSidebar")}</TooltipContent>
          </Tooltip>
        )}
        {isChildSession && parentSessionId && (
          <>
            ...
            >
              <Link to={`/c/${parentSessionId}`} aria-label={t("shell.backToParentSession")}>
            ...
            <div className="flex min-w-0 items-center gap-2">
              ...
              {boundAgent?.name ? (
                <div className="flex min-w-0 flex-col leading-tight">
                  ...
                  <span className="text-xs text-muted-foreground">{t("shell.subAgent")}</span>
                </div>
              ) : (
                <span className="text-sm font-semibold text-foreground">{t("shell.subAgent")}</span>
              )}
            </div>
          </>
        )}
        ...
        {canShare && shareDisabled && shareDisabledReason ? (
          <Tooltip>
            <TooltipTrigger asChild>
              ...
              <span
                tabIndex={0}
                aria-label={t("shell.shareDisabled", { reason: shareDisabledReason })}
            ...
            <DropdownMenu>
              <DropdownMenuTrigger asChild>
                <Button
                  ...
                  variant="ghost"
                  size="icon"
                  aria-label={t("shell.openSessionMenu")}
                  className="text-muted-foreground hover:text-foreground md:hidden"
                >
                  <PanelRightIcon className="size-4" />
                </Button>
