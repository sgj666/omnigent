...
export function validateTeamForm(values: Pick<TeamFormValues, "name" | "members">): string[] {
  ...
  if (
    values.members.some(
      (member) => !Number.isInteger(member.concurrency) || (member.concurrency ?? 0) <= 0,
    )
  ) {
...
export function TeamForm({
  ...
  const [members, setMembers] = useState<DraftMember[]>(
    initial?.members ?? [defaultMember("coordinator", "coordinator")],
  );
  const [workspaceId, setWorkspaceId] = useState(initial?.workspaceId ?? "");
  const [executionMode, setExecutionMode] = useState<TeamFormValues["executionMode"]>(
    initial?.executionMode ?? "auto",
  );
        ...
        <select
          ...
          onChange={(event) =>
            setMembers(
              members.map((member) => ({
                ...member,
                role: member.id === event.target.value ? "coordinator" : "worker",
              })),
            )
          }
        >
      ...
      <div className="grid gap-3 sm:grid-cols-2">
        <label className="text-sm font-medium">
          ...
          <Select
            ...
            onValueChange={(value) => setWorkspaceId(value === "none" ? "" : value)}
          >
            <SelectTrigger aria-label="Workspace">
              <SelectValue placeholder="Select workspace" />
            </SelectTrigger>
            <SelectContent>
              <SelectItem value="none">No workspace selected</SelectItem>
        ...
        <label className="text-sm font-medium">
          ...
          <Select
            value={executionMode}
            onValueChange={(value) => setExecutionMode(value as TeamFormValues["executionMode"])}
          >
            ...
            <SelectContent>
              ...
              <SelectItem value="readonly">Read-only analysis</SelectItem>
            </SelectContent>
          </Select>
        </label>
      </div>
