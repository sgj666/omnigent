...
class _GenerateBuildInfo(build_py):
    ...
    def _bundle_examples(self) -> None:
        ...
        ``omnigent/resources/examples/{polly,debby,zhuanharness}`` may exist as symlinks
        ...
        ``debby`` / ``zhuanharness`` subdirs, and bare ``omnigent`` (first-run default → polly)
        ...
        for name in ("debby", "polly", "zhuanharness"):
